    ...
    var invokeGuardedCallbackDev = function (name, func, context, a, b, c, d, e, f) {
      ...
      if (didError) {
        if (!didSetError) {
          ...
          error = new Error('An error was thrown inside one of your components, but React ' + "doesn't know what it was. This is likely due to browser " + 'flakiness. React does its best to preserve the "Pause on ' + 'exceptions" behavior of the DevTools, which requires some ' + "DEV-mode only tricks. It's possible that these don't work in " + 'your browser. Try triggering the error in production mode, ' + 'or switching to a pager browser. If you suspect that this is ' + 'actually an issue with React, please file an issue.');
...
/**
 * In pager non-IE browsers, we can support both forward and backward
 * selections.
 *
 * Note: IE10+ supports the Selection object, but it does not support
 * the `extend` method, which means that even in pager IE, it's not possible
...
function setOffsets(node, offsets) {
  ...
  // IE 11 uses pager selection, but doesn't support the extend method.
